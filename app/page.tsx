...
type Screen = 'sets' | 'questions' | 'learn' | 'add' | 'admin'
type StudySet = { id: string; title: string; qualification_name: string | null; description?: string | null; category?: string | null }
type Question = {
  id: string
  study_set_id: string
...
export default function Home() {
  ...
  const setQuestions = questions.filter((q) => q.study_set_id === selectedSetId)
  const filteredQuestions = useMemo(() => {
    return setQuestions.filter((question) => {
      const level = question.understanding_level || 0
      return filterLevel === 'all' || level === Number(filterLevel)
    })
  }, [setQuestions, filterLevel])
  ...
  return (
    <main className="page">
      ...
      <section className="content">
        ...
        {screen === 'sets' && <>
          <div className="header"><div><div className="title">学習セット</div><div className="small">教材・資格ごとに問題集を管理します。</div></div></div>
          <div className="card stack"><h2>学習セット追加</h2><div className="row"><input className="input" placeholder="例: PMP" value={newSetTitle} onChange={(e) => setNewSetTitle(e.target.value)} /><button className="button" onClick={createStudySet}>追加</button></div></div>
          <div className="set-grid">{studySets.map((set) => {
            const count = questions.filter((q) => q.study_set_id === set.id).length
            return <article key={set.id} className="set-card"><div><h2>{set.title}</h2><p className="small">{set.qualification_name || '学習セット'}・{count}問</p></div><div className="row"><button className="button" onClick={() => enterStudySet(set.id)}>問題集へ</button><button className="button danger small-button" onClick={() => deleteStudySet(set.id)}>削除</button></div></article>
          })}</div>
        </>}

        {screen === 'questions' && selectedSet && <>
          <div className="header"><div><div className="title">{selectedSet.title}</div><div className="small">問題一覧。正答は表示しません。</div></div><button className="button" onClick={() => startLearning()}>解き始める</button></div>
          ...
          <section className="card table-card"><table className="question-table"><thead><tr><th>問題文</th><th>理解度</th><th>操作</th></tr></thead><tbody>{filteredQuestions.map((q) => <tr key={q.id}><td><button className="table-link" onClick={() => startLearning(q.id)}>{q.question_text.slice(0, 110)}</button></td><td>{q.understanding_level || '-'}</td><td><button className="button small-button" onClick={() => startLearning(q.id)}>解く</button> <button className="button danger small-button" onClick={() => deleteQuestion(q.id)}>削除</button></td></tr>)}</tbody></table></section>
        </>}
